...
dicoRelations = {"r_syn": '5', "r_isa": '6', "r_anto": '7', "r_hypo": '8', "r_has_part": '9', "r_carac": '17', "r_holo": '10', 
                "r_agent-1": '24', "r_instr-1": '25', "r_patient-1": '26', "r_lieu-1": '28', "r_lieu" : '15'}
...
def getPolysemicTerm(dfEntries, name):
    polysemicPattern = ".+?(?=>)"
    prepro = re.search(polysemicPattern, name)
    name = '^' + prepro.group() + "[a-zA-Z ]*"
    serie = dfEntries["formated name "].str.contains(name)
    polysIndex = serie[serie == True].index[0]
    polysemicName = dfEntries["formated name "][polysIndex]
